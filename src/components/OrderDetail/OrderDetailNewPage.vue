<template>
  <div class="OrderDetailNewPage">
    <section class="OrderDetailContent">
      <!--订单状态部分-->
        ...
        <div class="goodsDetail">
          ...
          <div v-show="showGoodsDetail">
            ...
            <div v-if="isAlipayLife">
              ...
              <div class="order-flexBox">
                <div class="order-flexBox-left">已冻结押金</div>
                <div class="order-flexBox-right line-limit-length">￥{{ billInfo.payDepositAmt }}</div>
              </div>

              <div class="order-flexBox">
                <div class="order-flexBox-left">冻结押金方式</div>
                <div class="order-flexBox-right line-limit-length">花呗冻结押金 <i class="icon iconfont icon-help1" style="color: #007AFF; font-size: 14px" @click="showHuabeiTips"></i></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="orderDetail" v-show="showBillDetailEnable">
        ...
        <tab :line-width=3 custom-bar-width="60px" active-color='#FFDA29' v-model="orderPlanIndex">
          <tab-item class="vux-center" :selected="orderPlanIndex === item" v-for="(item, index) in orderMenuList" @click="orderPlaneDefault = item" :key="index">{{item}}
          </tab-item>
        </tab>
        <!--租赁信息-->
        ...
        <div v-if="reletPaySuccess" class="releted-detail">
          ...
          <div class="item-section bottom1pxline">
            <template v-if="reletDetailedVO.trialRentPaymentScheduleList && reletDetailedVO.trialRentPaymentScheduleList.length > 0">
              <div v-for="feeItem in reletDetailedVO.trialRentPaymentScheduleList" :key="feeItem.feeNo" v-if="feeItem.feeNo" class="item-row">
                <div class="item-col">
            ...
            <div class="item-row">
              <div class="item-col">
                <div class="rd-label-3">物主卡抵扣</div>
              </div>
              <div class="item-col text-right">
                <div class="rd-label-3">￥{{reletDetailedVO.cashOffsetAmt|moneyFormat}}</div>
              </div>
            </div>
            <div class="item-row">
              <div class="item-col">
...
class OrderDetail {
  ...
  getOrderDetail() {
    ...
    switch (this.status) {
      ...
      case orderStatus.AlreadyPayment: {
        staturTitle = '已支付';
        statusDesc =
          '您的租赁订单已首付完成，平台将尽快为您安排发货，请您耐心等待，注意发货通知短信哦。若有疑问，请致电客服：400-839-6296';
        break;
      }
      ...
      case orderStatus.AlreadyConsignment: {
        staturTitle = '已发货';
        statusDesc =
          '您的租赁商品已出库，快递正在配送中；请您提前准备好身份证原件及学生证原件用于收货时身份核验，并保持电话畅通，谢谢！若有疑问，请致电客服：400-839-6296';
        break;
      }
      case orderStatus.Leasing: {
        if (this.detailStatus === '7006') {
          staturTitle = this.statusDesc || '逾期-严重逾期';
          statusDesc = '您的租赁订单已严重逾期，请立刻支付相关费用！';
        } else {
          staturTitle = '租赁中';
          statusDesc = '您的商品已经收货完成，尽情享用吧！';
        }
        if (this.comingDue === 'Y') {
          // 说明是即将到期 是否即将到期：Y=是；N=否
          staturTitle = '即将到期';
          statusDesc =
            '您的订单还差' +
            '<span style="color: #FF4754">' +
            this.comingDueDays +
            '</span>' +
            '天就要到期了，请提前安排使用计划。';
        }
      ...
      case orderStatus.Rejected: {
        staturTitle = '已拒收';
        statusDesc = '拒收时，来回邮费由用户承担。邮费代扣成功后，订单将自动完结。';
        break;
      }
      default: {
  ...
  getFirBillDetail() {
    ...
    if (billStatus === repaymentStatus.WaitPayment) {
      // 支付状态为未支付
      paypayDate = '--';
    }
    let firstBill = {
  ...
  getProtocolInfoList() {
    ...
    if (this.status === orderStatus.WaitForConfirmed) {
      ...
      protocolList.push({
        ...
        url: currentPath + '/doc/wx/user_lease_agreement.htm'
      });
      protocolList.push({
        templateName: '用户租赁及服务协议之补充协议',
        url: currentPath + '/doc/wx/user_lease_supplementary_agreement.htm'
      });
    } else {
...
export default {
  ...
  computed: {
    ...
    canShowPayBtn: function() {
      if (this.orderDetail.status === orderStatus.WaitForPayment) {
        ...
        if (this.orderDetail.status === orderStatus.Leasing && this.detailStatus !== '7007') {
          if (this.orderDetail.payStyleStr === '按月分期') {
            // 如果是分期或者可以使分期或者一次性支付
            // return true
            // 一期该功能未开发，所以隐藏支付按钮
            return true;
    ...
    canShowFixBtn: function() {
      // 增加7007隐藏所有的支付按钮
      if (
        (this.orderDetail.status === orderStatus.Leasing ||
          this.orderDetail.status === orderStatus.DueSoon ||
          this.orderDetail.status === orderStatus.BuyOuting) &&
        this.detailStatus !== '7007'
      ) {
        return true;
      } else {
        return false;
    ...
    canShowFirstBillView: function() {
      if (
        this.orderDetail.status === orderStatus.WaitForPayment ||
        this.orderDetail.status === orderStatus.AlreadyPayment ||
        this.orderDetail.status === orderStatus.WaitForConsignment ||
        this.orderDetail.status === orderStatus.AlreadyConsignment
      ) {
        return true;
      } else {
        return false;
  ...
  methods: {
    ...
    updateOrderPageManager: function(pushState, increaseMent, cleanCount) {
      // 每次+2 用于计算层数
      let originCount =
        this.$store.state.orderDetailPageManager && this.$store.state.orderDetailPageManager.orderDetailCount;
    ...
    getOrderPageManagerDetail: function() {
      let pushState =
        this.$store.state.orderDetailPageManager && this.$store.state.orderDetailPageManager.orderDetailPush;
      let pushCount =
        this.$store.state.orderDetailPageManager && this.$store.state.orderDetailPageManager.orderDetailCount;
      if (pushCount === undefined) {
        pushCount = 0;
    ...
    getHttpOrderDetail: function(orderNo) {
      // orderNo = '00420181116M045790059983';
      let that = this;
      let param = {
      ...
      that.$http.get(orderDetailUrl, param).then(res => {
        ...
        if (res.code === '00') {
          ...
          that.billInfo = myOrderDetail.billInfo;
    ...
    menuButtonClick(menuItem) {
      ...
      switch (menuItem) {
        ...
        case '归还': {
          // 跳转到归还页面
          this.$router.push({ name: 'GoodsReturn', params: { orderNo: this.orderNo } });
          break;
        }
        case '续租': {
          let paramItem = {
    ...
    showHuabeiTips() {
      this.$vux.alert.show({
        title: '花呗预授权',
        content: '使用花呗额度冻结押金。订单完结后，押金将自动解冻。',
        onShow() {},
        onHide() {}
      });
    },
