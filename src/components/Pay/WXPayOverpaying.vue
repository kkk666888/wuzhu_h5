<template>
	<div class="WXPayOverpaying">
     ...
     <div class="box-ticket-number">
       <p class="ticket-number-text">订单编号:<span>{{ dataOrderBillPayInfos.orderNo }}</span></p>
     </div>
     ...
     <div class="box-goods-more">
      ...
      <div class="box-goodsDetail">
        ...
        <span class="label" v-for="item in dataOrderBillPayInfos.specContentList.split(',')"> {{ item }} </span>
        </p>
      </div>
      </div>
		</div>
    ...
    <ul class="clearFix">
    <li v-for="(item,index) in dataOrderBillPayInfos.rentBillList">
    ...
    <div class="box-pay-first-amount box-ticket-two-line">
      ...
      <div class="box-pay-amount">
        <p class="box-pay-amount-p2">¥{{ item.totalPayAmt }}
    ...
    <div v-show="item.isDown" :class="['box-pay-ticket', 'box-ticket-down']">
      <div v-for="itemSub in item.billDetailList">
      <div class="clearFix">
    ...
    <!-- 第一期不上 -->
		...
		<div>
		  <group gutter="18px">
      <cell class="amount-descount" title="账户抵扣" :value="dataOrderBillPayInfos.integralAmt"></cell>
...
<script>
  ...
  methods: {
    ...
    toPayCash() {
      ...
      this.$store.commit('updateDeductionAmount', { deductionAmount: this.dataOrderBillPayInfos.integralAmt })
    ...
    getOrderBillPayInfos(orderNo) {
      ...
      this.$http.get('/wuzhu/order/getOrderBillPayInfos?orderNo=' + orderNo).then(res => {
        ...
        if (res.code === '00') {
          ...
          this.dataOrderBillPayInfos.sumAmtChar = '¥' + this.dataOrderBillPayInfos.sumAmt
          if (this.dataOrderBillPayInfos.rentBillList) {    // 为账单插入是否展开字段
  ...
  mounted() {
    ...
    // orderNo = '00120180525000000160086678'    // 程罗订单
    // orderNo = '00120180522000000161789891'    // 账单测试订单
    // orderNo = '00120180526000000114416855'    // 余兵测试订单
    // orderNo = '00120180531M000162827470'    // 程罗租赁中订单
    // orderNo = '00120180705M000176495971'    // 异常测试订单号，异常无法跳转订单
...
//   background-image: url('./../../assets/inputradio.gif');
...
// label input[type="radio"] + span{width:20px;height:20px;display:inline-block;background:url(http://www.haorooms.com/uploads/images/inputradio.gif)  no-repeat;background-position:0 0;}
// label input[type="radio"]:checked + span{background-position:0 -21px}
}
</style>
