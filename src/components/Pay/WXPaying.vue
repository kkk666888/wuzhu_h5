  ...
  <div class="WXPaying">
    ...
    <div class="box-goods">
      ...
      <div class="box-goodsDetail">
        ...
        <p class="goods-price">商品价值：
          <span>{{ dataOrderFirstPayInfo.marketPrice }}</span>元</p>
        <p class="goods-labels">
          <span class="label" v-for="(item,index) in dataOrderFirstPayInfo.specContentList.split(',')" :key="index">
            {{ item }}
          </span>
        </p>
      </div>
    ...
    <div v-show="!isShow" class="boxFormGoodsInfo">
      ...
      <!-- 按期支付 -->
      <div v-show="(dataOrderFirstPayInfo.monthlyPayStyle === '0') || (dataOrderFirstPayInfo.monthlyPayStyle === '0,1' && dataLogic.isTerm)" v-for="(item, index) in dataOrderFirstPayInfo.firstFeeList" :key="index" class="box-form-item clearFix">
      ...
      <!-- 订单支付 -->
      <div v-show="(dataOrderFirstPayInfo.monthlyPayStyle === '1') || (dataOrderFirstPayInfo.monthlyPayStyle === '0,1' && !dataLogic.isTerm)" v-for="(item,index) in dataOrderFirstPayInfo.oneAllPayFeeList" :key="index" class="box-form-item clearFix">
        <div v-if="item.feeName.indexOf('违约金') === -1 && item.feeName.indexOf('滞纳金') === -1">
          <div class="box-form-item-label">
            <div>{{ item.feeName }}</div>
          </div>
    ...
    <div class="box-isAgree">
      <p class="input-tips" @click="agreementRadioClick">
        ...
        <span v-for="(item,index) in dataOrderFirstPayInfo.pdfFileList" :class="['rent-agreement', 'rent-agreement' + index]" :key="index">《{{ item.templateName }}》</span>
  ...
  methods: {
    ...
    toPayCash() {
      ...
      if (this.isAgree) {
        ...
        let payAmount = 0;
        if (
          this.dataOrderFirstPayInfo.monthlyPayStyle === '0' ||
          (this.dataOrderFirstPayInfo.monthlyPayStyle === '0,1' && this.dataLogic.isTerm)
        ) {
