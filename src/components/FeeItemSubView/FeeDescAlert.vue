  ...
  <div>
    <div v-transfer-dom>
      <x-dialog v-model="showScrollBox" class="dialog-demo" :isPreventDefault="false" @fs-toggle="fsToggle">
        ...
        <!--<div @click="showScrollBox=false" style="background: palegoldenrod">X</div>-->
      ...
      methods: {
        ...
        fsToggle(that, val) {
          ...
          that.$nextTick(function() {
            ...
            if (_scrollHeight > _clientHeight) { // 有 滚条
            } else {
              if (_that.isShowFirst) {
                _that.isShowFirst = false
                that.$el.querySelector('.weui-dialog').addEventListener('touchmove', function(e) {
