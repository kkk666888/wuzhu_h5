    ...
    <section class="home-content">
      ...
      <div>
        <swiper :aspect-ratio="400/750" loop auto :index="banner_index" dots-position="center">
          <swiper-item class="fs-swiper-img" v-if="item.type != '2'" v-for="(item, index) in banner_list" :key="index" @click.native="bannerClickIndex(index, item)">
      ...
      <div class="rent_progress vux-1px-b">
        <img src="./img/liucheng_2.png">
      </div>
      <!-- 活动图片 -->
      <div v-if="activityItem && activityItem.type === '2'" class="activity-div" @click="goodsSectionClick(activityItem, 'activeClick')">
      ...
      <div v-if="popularList && popularList.length > 0" class="hot-div">
        <div class="header"><span class="hot-span"></span>热租爆款</div>
        <div class="item-row" v-for="item in popularList" :key="item.categoryCode" @click="goodsClick(item.categoryCode, 'homeClick')">
          <div class="item-col">
      ...
      <div class="recommend-div" v-for="item0 in phoneList" :key="item0.configSerialNo">
        <div class="header">
          {{item0.title}}&nbsp;
          <span v-if="item0.typeNo" class="more" @click="goodsSectionClick(item0, 'floorClick')">
            更多<img class="right" src="./img/right.png" />
          </span>
        </div>
        <div class="item-row">
          <div class="item-col" v-for="item in item0.listCommodityCategory" :key="item.categoryCode" @click="goodsClick(item.categoryCode, 'homeClick')">
